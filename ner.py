...
ner = pipeline(
    "token-classification", model="dslim/bert-large-NER", aggregation_strategy="simple"
)
...
def row2NamedEntities(row):
    ...
    for result in ner_results:
        entities = entities + [
            {"name": result["word"], "entity": result["entity_group"], **metadata}
        ]
...
def dfText2DfNE(dataframe):
    ...
    entities_dataframe = (
        entities_dataframe.groupby(["name", "entity", "chunk_id"])
        .size()
